...
def process_coupons() -> Dict[str, Any]:
    ...
    for coupon in coupons:
        ...
        if not fcm_tokens:
            ...
            # 5. 실패한 토큰 삭제 (좀비 토큰 정리)
            if result.get('failed_tokens'):
                deleted_count = firestore_service.delete_fcm_tokens_batch(
                    result['failed_tokens']
                )
                logger.info(f"Deleted {deleted_count} expired FCM tokens from Firestore")
...
if scheduler_fn:
    ...
    def manual_coupon_scraper(req: https_fn.Request) -> https_fn.Response:
        ...
        Query Parameters:
            ...
            test: 'true'로 설정하면 테스트 알림만 전송 (실제 스크래핑 안함)
        ...
        try:
            ...
            is_test_mode = req.args.get('test', '').lower() == 'true'
            ...
            if is_test_mode:
                ...
                logger.info(f"Sending test notification to {len(fcm_tokens)} users")
